...
async function init() {
    ...
    if (addBtn && !['admin', 'staff'].includes(currentUser.role)) addBtn.style.display = 'none';
...
async function loadEquipment() {
    const { data } = await supabase.from('equipment').select('*').order('id', { ascending: false });
    allEquipment = data || [];
    renderTable();
}
...
function renderTable() {
    ...
    if (allEquipment.length === 0) {
        body.innerHTML = `<tr><td colspan="6" class="empty-row">No equipment</td></tr>`;
        return;
    }
    ...
    body.innerHTML = allEquipment.map(e => {
        let actions = '';
        if (canManage) actions += `<button class="btn btn-info btn-sm" onclick="editEq(${e.id})">✏️ Edit</button> `;
        if (canDelete) actions += `<button class="btn btn-danger btn-sm" onclick="deleteEq(${e.id})">🗑️ Delete</button>`;
        if (isRequester && e.status === 'Available')
            actions += `<a href="my-requests.html?equipment_id=${e.id}" class="btn btn-success btn-sm">📤 Request</a>`;
        if (isRequester && e.status !== 'Available')
            actions += `<span style="color:#999;font-size:0.8rem;">👁️ View only</span>`;
        if (!actions) actions = '<span style="color:#999;font-size:0.8rem;">👁️ View only</span>';
...
window.deleteEq = async (id) => {
    ...
    await logAudit('DELETED', 'Equipment', id, `Deleted #${id}`);
    await loadEquipment();
};
...
function setupEvents() {
    ...
    if (closeBtn) closeBtn.onclick = () => document.getElementById('equipmentModal').style.display = 'none';
